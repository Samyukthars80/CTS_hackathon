...
def train_pharmacy_model() -> dict:
    print("\n" + "="*50)
    print("TRAINING PHARMACY CLAIMS ANOMALY MODEL")
    print("="*50)
    
    # 1. Load cleaned data
    file_path = DATASETS["PHARMACY_CLAIM"]
    if not file_path.exists():
        raise FileNotFoundError(f"Cleaned pharmacy claim file not found: {file_path}")
        
    df = pd.read_csv(file_path)
    print(f"Loaded Pharmacy Claims. Shape: {df.shape}")
    ...
    feat_cfg = FEATURES["PHARMACY_CLAIM"]
    ...
    model_path = MODELS_DIR / "pharmacy_isolation_forest.pkl"
    prep_path = MODELS_DIR / "pharmacy_preprocessor.pkl"
    feat_path = MODELS_DIR / "pharmacy_features.json"
