...
def train_prior_auth_model() -> dict:
    print("\n" + "="*50)
    print("TRAINING PRIOR AUTHORIZATION ANOMALY MODEL")
    print("="*50)
    
    # 1. Load cleaned data
    file_path = DATASETS["PRIOR_AUTH"]
    if not file_path.exists():
        raise FileNotFoundError(f"Cleaned prior auth file not found: {file_path}")
        
    df = pd.read_csv(file_path)
    print(f"Loaded Prior Auths. Shape: {df.shape}")
    ...
    model_path = MODELS_DIR / "prior_auth_isolation_forest.pkl"
    prep_path = MODELS_DIR / "prior_auth_preprocessor.pkl"
    feat_path = MODELS_DIR / "prior_auth_features.json"
